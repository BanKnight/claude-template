---
name: setup-workflow
description: 初始化或更新本工作流模板项目的 `.workflow/` 与 `docs/` 双区结构，并注入 `CLAUDE.md` / `AGENTS.md` 治理加载规则。用户要求 setup、初始化、更新、修复、重新执行工作流结构、治理文件、模板目录或 docs 索引时使用。
---
...
当用户要求 setup、初始化、更新、修复、重新执行工作流结构，或要求补齐治理文件、模板目录、docs 索引、`AGENTS.md` / `CLAUDE.md` 加载规则时，使用本技能。
...
执行 setup 时，从本 skill 的 `templates/workflow/` 与 `templates/docs/` 补齐项目根目录下的 `.workflow/` 与 `docs/`。
