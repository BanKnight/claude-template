...
`help-workflow` 用于介绍整套工作流，并根据用户描述推荐合适的 workflow 命令。

它是工作流说明和分流入口，不是项目状态诊断命令。
...
- 更新 `.workflow/` 或 `docs/`。
- 创建、修改或移动任何 artifact。
- 代替其他 workflow 命令执行实际工作。

如果用户询问“当前项目现在到哪了”或“现在具体该做哪个 change”，不要自行读取 roadmap/change/archive 做状态恢复；应说明这不是 `help-workflow` 的职责，并建议用户进入对应命令或明确要求状态检查。
...
不要在用户已经明确要求执行具体命令时插入 `help-workflow`。
