...
如果实现过程中发现 plan/spec/design/tasks 有问题，应暂停并建议回到对应命令修正，而不是在实现阶段偷偷改写上游 artifact。
...
6. 对标记为可并行的任务，只有在它们不修改同一文件且依赖已满足时，才可以并行或连续处理。
7. 每次只围绕当前任务做最小必要代码变更。
8. 完成任务后立即在 `tasks.md` 中把对应任务从 `- [ ]` 改为 `- [x]`。
9. 如果任务完成需要补充实现说明，只写入任务下方的简短结果，不写长篇复盘。
10. 继续下一个任务，直到所有任务完成、遇到阻塞、用户中断或需要回到上游 artifact。
...
实现过程中要反思 `plan-change` 的产出是否足够支持实现。

如果发现以下问题，应暂停并建议优化 `plan-change` 或其模板：

- plan 没有列出必须读取的长期 docs。
- tasks 没有写清依赖关系。
- tasks 没有标明可并行性或同文件冲突。
- tasks 粒度过大，无法独立完成和验收。
- tasks 缺少验收标准。
- tasks 无法追溯到 spec/design/plan。
- plan 没有说明 design 缺省理由。
- plan 没有列出验证关注点，导致实现后不知道如何检查。

如果只是当前 change 的 plan/tasks 问题，建议回到 `plan-change` 更新该 change；如果是模板或命令规则问题，记录为后续改进工作。
